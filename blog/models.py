...
class Category(models.Model):
    ...
    slug = AutoSlugField(populate_from="title", unique=False, primary_key=False)
...
class Post(models.Model):
    ...
    title = models.CharField(max_length=255, blank=False, null=False)
    slug = AutoSlugField(
        _("slug"), populate_from="title", unique=True, primary_key=False
    )
    category = models.ForeignKey(
        Category, blank=True, null=True, on_delete=models.SET_NULL, related_name="posts"
    )
    content = CKEditor5Field(
        "Text", blank=True, null=True, config_name="extends"
    )  # RichTextUploadingField(_('content'), blank=True, null=True)
